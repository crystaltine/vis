...
def draw_menu_background(filename:str): 
...
def create_menu(): 
          ...
          draw_menu_background('assets/level_editor_menu/level_editor_menu.png')
          draw_menu_title()
          draw_all_buttons(1) 
...
def handle_main_page_movement(val):
        ...
        if val.name=='KEY_LEFT':
            ...
            if current_button_index<0:
                current_button_index=5

        if val.name=='KEY_RIGHT':
            changed=True
            current_button_index+=1
            if current_button_index>5:
                current_button_index=1

        if val.name=='KEY_ENTER': 
            draw_menu_background('assets/level_editor_menu/red_bg.png')
        ...
        if changed and not val.name =='KEY_ENTER' and not val.name == 'KEY_ESCAPE':

    ...
    for root, dirs, files in os.walk('levels'):
        ...
        for file in files:
            ...
            if file.endswith(file_type):
                ...
                level_file_names.append(os.path.join(root, file))
                logging.error(level_file_names)
...
def draw_created_levels():
    ...
    with GD.term.hidden_cursor():
        ...
        if level_path=='You have not created any levels':
            ...
            draw_created_level(level_path[level_path.index('\\')+1:level_path.index('.level')], int(GD.term.width*0.8), int(GD.term.height*0.6), int(GD.term.width*0.1), int(GD.term.height*0.3), color[0], color[1])
...
def init_created_levels_page(terminal):
    ...
    if level_path=='You have not created any levels':
        ...
        draw_created_level(level_path[level_path.index('\\')+1:level_path.index('.level')], int(terminal.width*0.8), int(terminal.height*0.6), 

    ...
    if orientation == 'right':
        ...
        for i in range(y, y+height):
            ...
            for j in range(height-(i-y)):
                spaces+=f"{GDConstants.term.on_black} "
            dots=""
...
def draw_bottom_menu():
   ...
   # This clears the screen
    ...
    draw_rect('black', Position.Relative(left=0, top=int(GDConstants.term.height*0.6)), GDConstants.term.width, int(GDConstants.term.height*0.41)+1)
    ...
    while True:
        ...
        draw_text(title_text, int((GDConstants.term.width-len(title_text))/2)-1,int(GDConstants.term.height*0.63), True)

        # Writing WASD bindings to the screen

        wasd_text='Move Screen:'
        draw_text(wasd_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.68))

        w_text='   Up: [W]'
        draw_text(w_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.7))

        a_text='   Left: [A]'
        draw_text(a_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.72))

        s_text='   Down: [S]'
        draw_text(s_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.75))

        d_text='   Right: [D]'
        draw_text(d_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.77))

        # Writing arrow key bindings to the screen

        wasd_text='Move Selected Object:'
        draw_text(wasd_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.81))

        up_text='   Up: [↑]'
        draw_text(up_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.83))

        left_text='   Left: [←]'
        draw_text(left_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.85))

        down_text='   Down: [↓]'
        draw_text(down_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.89))

        right_text='   Right: [→]'
        draw_text(right_text, int(GDConstants.term.width*0.02), int(GDConstants.term.height*0.9))

        # Drawing the block onto the screen

        draw_rect('red', Position.Relative(left=int(GDConstants.term.width*0.25), top=int(GDConstants.term.height*0.68)), int(GDConstants.term.width*0.08), int(GDConstants.term.height*0.14))

        # Writing the "Toggle Block" text onto the screen

        block_text='Toggle Block: [B]'
        draw_text(block_text, int(GDConstants.term.width*0.245), int(GDConstants.term.height*0.83))

        # Drawing a spike onto the screen

        draw_spike(int(GDConstants.term.height*0.14), int(GDConstants.term.width*0.39), int(GDConstants.term.height*0.68), 'green')

        # Writing the "Toggle Spike" text onto the screen

        spike_text='Toggle Spike: [S]'
        draw_text(spike_text, int(GDConstants.term.width*0.385), int(GDConstants.term.height*0.83))

        # Drawing a blue orb onto the screen

        draw_orb(int(GDConstants.term.width*0.07), int(GDConstants.term.height*0.14), int(GDConstants.term.width*0.54), int(GDConstants.term.height*0.68), 'turquoise')

        # Writing the "Toggle Blue Orb" text onto the screen

        blue_orb_text='Toggle Blue Orb: [O]'
        draw_text(blue_orb_text, int(GDConstants.term.width*0.52), int(GDConstants.term.height*0.83))

        # Drawing a purple orb onto the screen

        draw_orb(int(GDConstants.term.width*0.07), int(GDConstants.term.height*0.14), int(GDConstants.term.width*0.69), int(GDConstants.term.height*0.68), 'purple')

        # Writing the "Toggle Purple Orb" text onto the screen

        purple_orb_text='Toggle Purple Orb: [P]'
        draw_text(purple_orb_text, int(GDConstants.term.width*0.665), int(GDConstants.term.height*0.83))

        # Drawing a yellow orb onto the screen

        draw_orb(int(GDConstants.term.width*0.07), int(GDConstants.term.height*0.14), int(GDConstants.term.width*0.845)+1, int(GDConstants.term.height*0.68), 'yellow')

        # Writing the "Toggle Yellow Orb" text onto the screen

        purple_orb_text='Toggle Yellow Orb: [Y]'
        draw_text(purple_orb_text, int(GDConstants.term.width*0.825), int(GDConstants.term.height*0.83))

        # Writing other bindings below the shapes

        save_text='Save Level: [X]'
        draw_text(save_text, int(GDConstants.term.width*0.245), int(GDConstants.term.height*0.9))

        load_text='Load Level: [L]'
        draw_text(load_text, int(GDConstants.term.width*0.39), int(GDConstants.term.height*0.9))

        undo_text='Undo: [Z]'
        draw_text(undo_text, int(GDConstants.term.width*0.55), int(GDConstants.term.height*0.9))

        clear_text='Clear All: [C]'
        draw_text(clear_text, int(GDConstants.term.width*0.685), int(GDConstants.term.height*0.9))

        test_text='Test Level: [T]'
        draw_text(test_text, int(GDConstants.term.width*0.84), int(GDConstants.term.height*0.9))

        toggle_placement_text='Object Placement/Pickup: [Enter]'
        draw_text(toggle_placement_text, int(GDConstants.term.width*0.265), int(GDConstants.term.height*0.96))

        delete_text='Delete Selected Object: [Backspace]'
        draw_text(delete_text, int(GDConstants.term.width*0.63), int(GDConstants.term.height*0.96))

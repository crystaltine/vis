...
class EditColorPopup:
    ...
    def __init__(
        ...
        frame: "CameraFrame", 
        level: "Level",
        init_target: Literal["bg", "grnd"] | int):

        
        self.target_channel = init_target
        ...
        self.inputs = [str(c) for c in level.get_color_channel(self.target_channel)]
        ...
        self.height = round(max(
            EditColorPopup.FRACTION_OF_SCREEN_HEIGHT * frame.height,
            TextureManager.font_small1.font_height*6 + EditColorPopup.PADDING_Y*2 + EditColorPopup.GAP_Y*5
        ...
        self.square_sidelen = self.height - TextureManager.font_small1.font_height*3 - EditColorPopup.PADDING_Y*2 - EditColorPopup.GAP_Y*3
        
        self.width = round(max(
            EditColorPopup.FRACTION_OF_SCREEN_WIDTH * frame.width,
            TextureManager.font_small1.font_width*11 + EditColorPopup.PADDING_X*2, # be able to fit title
            self.square_sidelen + EditColorPopup.PADDING_X*4 + TextureManager.font_small1.font_width*4 # be able to fit color labels, square, etc.
        ))
    ...
    def render(self):
        ...
        label_left = self.left + EditColorPopup.PADDING_X
        input_right = self.right - EditColorPopup.PADDING_X
        
        if self.selected_option == 0: # draw bg spanning from label_left-PADDING_SELECTION to input_right+PADDING_SELECTION
            new_frame.add_rect(
                EditColorPopup.SELECTED_OPTION_BG_COLOR,
                label_left-EditColorPopup.PADDING_SELECTION, 
                self.top+TextureManager.font_small1.font_height + EditColorPopup.PADDING_Y+EditColorPopup.GAP_Y-EditColorPopup.PADDING_SELECTION,
                input_right-label_left+EditColorPopup.PADDING_SELECTION*2, 
                TextureManager.font_small1.font_height+EditColorPopup.PADDING_SELECTION*2
            )
        text_center = self.top+TextureManager.font_small1.font_height + EditColorPopup.PADDING_Y + EditColorPopup.GAP_Y + TextureManager.font_small1.font_height // 2
        ...
        preview_square_top = self.top + TextureManager.font_small1.font_height*2 + EditColorPopup.PADDING_Y + EditColorPopup.GAP_Y*2
        
        # draw preview square
        new_frame.add_rect(
            self.get_color(),
            self.left+EditColorPopup.PADDING_X+1, 
        ...
        text_right = self.right - EditColorPopup.PADDING_X
        text_label_left = self.left + self.square_sidelen + EditColorPopup.PADDING_X*2
        ...
        curr_text_top = preview_square_top + (self.square_sidelen - TextureManager.font_small1.font_height*3 - EditColorPopup.GAP_Y*2) // 2
        ...
        if self.selected_option == 1: # draw bg spanning from text_label_left-PADDING_SELECTION to right+PADDING_SELECTION
            new_frame.add_rect(
                EditColorPopup.SELECTED_OPTION_BG_COLOR,
                text_label_left-EditColorPopup.PADDING_SELECTION, curr_text_top-EditColorPopup.PADDING_SELECTION,
                text_right-text_label_left+EditColorPopup.PADDING_SELECTION*2, TextureManager.font_small1.font_height+EditColorPopup.PADDING_SELECTION*2
            )
        ...
        curr_text_top += TextureManager.font_small1.font_height + EditColorPopup.GAP_Y
        curr_text_center = curr_text_top + TextureManager.font_small1.font_height // 2
        ...
        text_center_y = self.bottom - TextureManager.font_small1.font_height//2 - EditColorPopup.PADDING_Y
        new_frame.add_rect(
            (95, 210, 91, 255),
            self.horiz_center,
            text_center_y,
            TextureManager.font_small1.font_width*4 + EditColorPopup.SAVE_BUTTON_PAD_X*2,
            TextureManager.font_small1.font_height + EditColorPopup.SAVE_BUTTON_PAD_Y*2,
            anchor="center",
            outline_width=int(self.selected_option == 4),
            outline_color=(255, 255, 255, 255)
        )
        new_frame.add_text(self.horiz_center, text_center_y, TextureManager.font_small1, "Save", anchor="center")
    ...
    def handle_key(self, key: "Keystroke") -> Literal["close"] | None:
        ...
        # L/R arrow keys - change the currently selected button
            ...
            if self.selected_option == 0:
                ...
                # load the actual color of that channel into self.inputs
                self.inputs = [str(c) for c in self.level.get_color_channel(self.target_channel)]

...
class Position:
    ...
    class Relative:
        ...
        def get_absolute(self, container_width: int = None, container_height: int = None) -> "Position.Absolute":
            ...
            If container width or height is not provided, the MM.term width and height will be used.
            """
            
            container_width = container_width or MM.term.width
            container_height = container_height or MM.term.height
...
def fcode(foreground: str | tuple[int] = None, background: str | tuple[int] = None, style: str = None) -> str:
    ...
    Having that `RS` code at the end is technically optional but without it the formatting will continue to be used by the MM.term.
...
def draw_rect(
    ...
    conv_width = convert_to_chars(MM.term.width, width) if width is not None else MM.term.width
    conv_height = convert_to_chars(MM.term.height, height) if height is not None else MM.term.height
    ...
    abs_pos = position.get_absolute(MM.term.width, MM.term.height)
    ...
    #Logger.log(f"conv_height, conv_width: {conv_height}, {conv_width}")
    true_top = abs_pos.top if abs_pos.top is not None else MM.term.height - abs_pos.bottom - conv_height
    true_left = abs_pos.left if abs_pos.left is not None else MM.term.width - abs_pos.right - conv_width
    
    # find true width and height
    true_width = MM.term.width - abs_pos.right - abs_pos.left if (abs_pos.left is not None and abs_pos.right is not None) else conv_width
    true_height = MM.term.height - abs_pos.bottom - abs_pos.top if (abs_pos.top is not None and abs_pos.bottom is not None) else conv_height
    ...
    # draw the rectangle
    with MM.term.hidden_cursor():
        for i in range(true_top, true_top + true_height):
            print2(MM.term.move_yx(i, true_left) + fcode(color) + "█"*true_width)
